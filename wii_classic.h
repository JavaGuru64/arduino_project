...
//  
//
//  Created by Joshua Long on 10/18/12.
//  based off code by Tod E. Kurt, http://todbot.com/blog/
//

#include <math.h>
#include <Arduino.h>

static uint8_t data_pack[6]; // Data retrieved from wii classic controller

// Setup the power pins manually.

// From the nunchuck_init() in Tod Kurt's nunchuck_funcs.h program.
...
// From the nunchuck_send_request() in Tod Kurt's nunchuck_funcs.h program.
...
// Same as the nunchuck_decode_byte() in Tod Kurt's nunchuck_funcs.h program.
static char decode_byte (char x)
{
    x = (x ^ 0x17) + 0x17;
    return x;
}

// From the nunchuck_get_data() in Tod Kurt's nunchuck_funcs.h program.
static int get_data()
{
    int cnt=0;
    Wire.requestFrom(0x52, 6); // Requests data
    ...
    while (Wire.available()) {
        ...
        data_pack[cnt] = decode_byte( Wire.read() );
        cnt++;
    }
    send_request();  // send request for next data payload
    
    // Checks to see if the data was recieved
    if (cnt >= 5) {
        return 1;   // success
    }
    return 0; //failure
